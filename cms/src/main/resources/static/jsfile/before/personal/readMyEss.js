...
function updateArticle() {
    window.location.href="updateEss.html?telphone=" + telphone +"&userFaceUrl="+userFaceUrl+"&article_pk="+article_pk;
}
...
function deleteArticle() {
    if (confirm("确定要删除？")){
        $.ajax({
            url: "../../../articleController/deleteArticleByPK.do",
            type: "POST",
            dataType: "json",
            data: "article_pk=" + article_pk,
            success: function (mu) {
                if (mu.m_istatus == 1) {
                    alert(mu.m_strMessage);
                    window.location.href="../module/index-login.html?telphone=" + telphone +"&userFaceUrl="+userFaceUrl;
                }else {
                    alert(mu.m_strMessage);
                }
            }
